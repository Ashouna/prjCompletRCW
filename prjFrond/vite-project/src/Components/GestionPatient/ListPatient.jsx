...
const ListPatient = () => {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:3305/api/patients/${id}`);
                    ...
                    <tbody>
                        {patients.map(patient => (
                            <tr key={patient.Id}>
                                ...
                                <td>
                                    <button onClick={() => handleView(patient.IdPatient)}>Voir</button>
                                    <button onClick={() => handleEdit(patient.IdPatient)}>Modifier</button>
                                    <button className="delete-button" onClick={() => handleDelete(patient.IdPatient)}>Supprimer</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
