...
namespace prjRcwfinalserver.Controllers
{
    ...
    public class PatientsController : ApiController
    {
        ...
        {
            var patient = await _context.Patients.Find(p => p.IdPatient == new ObjectId(id)).FirstOrDefaultAsync();
        ...
        public async Task<IHttpActionResult> Update(string id, Patient patientIn)
        {
            try
            {
                ...
                var existingPatient = await _context.Patients.Find(p => p.IdPatient == new ObjectId(id)).FirstOrDefaultAsync();
                ...
                patientIn.IdPatient = existingPatient.IdPatient;

                var result = await _context.Patients.ReplaceOneAsync(p => p.IdPatient == new ObjectId(id), patientIn);
        ...
        {
            var result = await _context.Patients.DeleteOneAsync(p => p.IdPatient == new ObjectId(id));
